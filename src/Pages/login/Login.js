        ...
        <div className='direita-login'>
          <div className='card-login'>
            ...
            <div className='campo-texto-login'>
              <input id='email' type="text" name="email" placeholder="E-mail" value={this.setState.email}
                onChange={event => this.setState({ email: event.target.value })} />
            </div>
            <div className='campo-texto-login'>
              <input id='password' type="password" name="password" placeholder="Senha" onKeyPress={this.handleEnter}
                onChange={event => this.setState({ password: event.target.value })} />
            </div>
            <button className='botao-esqueceu-senha'>Esqueceu a senha?</button>
            <button className='botao-login' onClick={this.entrar}>Entrar</button>
            <h3 className='h3-nao-tem-conta'>Não tem conta?</h3>
            <a className='botao-criar-conta' onClick={this.registration}>Registrar Agora</a>
          </div>
        </div>
      </div>
    )
